...
class OnModifierState {
  ...
  updateListener() {
    ...
    let eventName = debug.check(reference.valueForRef(args.positional[0]), debug.CheckString, () => 'You must pass a valid DOM event name as the first argument to the `on` modifier');
    util.assert(args.positional[1], 'You must pass a function as the second argument to the `on` modifier');
    let userProvidedCallback = debug.check(reference.valueForRef(args.positional[1]), debug.CheckFunction, actual => {
    ...
    if (env.DEBUG) {
      ...
      once = debug.check(_once, debug.CheckOr(debug.CheckBoolean, debug.CheckUndefined), actual => {
      ...
      passive = debug.check(_passive, debug.CheckOr(debug.CheckBoolean, debug.CheckUndefined), actual => {
      ...
      capture = debug.check(_capture, debug.CheckOr(debug.CheckBoolean, debug.CheckUndefined), actual => {
