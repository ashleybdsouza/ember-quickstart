    ...
    case HeadKind.Call:
      {
        ...
        let builtExpr = buildCallHead(path, trusted ? VariableResolutionContext.ResolveAsHelperHead : VariableResolutionContext.ResolveAsComponentOrHelperHead, symbols);
    ...
    case HeadKind.Block:
      {
        ...
        let path = buildCallHead(normalized.head, VariableResolutionContext.ResolveAsComponentHead, symbols);
...
function buildElementParams(attrs, symbols) {
  ...
  for (const [key, value] of Object.entries(attrs)) {
    if (value === HeadKind.Splat) {
      params.push([SexpOpcodes.AttrSplat, symbols.block('&attrs')]);
    } else if (key[0] === '@') {
      keys.push(key);
    ...
    case ExpressionKind.Literal:
      {
        ...
        if (val === false) {
          ...
          return [[SexpOpcodes.StaticAttr, name, '', namespace ?? undefined]];
        } else if (typeof val === 'string') {
          return [[SexpOpcodes.StaticAttr, name, val, namespace ?? undefined]];
        } else {
          throw new Error(`Unexpected/unimplemented literal attribute ${JSON.stringify(val)}`);
        }
      }
    default:
      return [[SexpOpcodes.DynamicAttr, name, buildExpression(value, 'AttrValue', symbols), namespace ?? undefined]];
  }
}
...
function buildVar(head, context, symbols, path) {
  ...
  switch (head.kind) {
    case VariableKind.Free:
      ...
      break;
    default:
      op = SexpOpcodes.GetSymbol;
      sym = getSymbolForVar(head.kind, symbols, head.name);
  }
  if (path === undefined || path.length === 0) {
    return [op, sym];
  } else {
    assert(op !== SexpOpcodes.GetStrictKeyword, '[BUG] keyword with a path');
    return [op, sym, path];
  }
}
...
class ContentEncoder {
  ...
  SimpleElement({
    ...
    return new WireStatements([[op, deflateTagName(tag.chars)], ...CONTENT.ElementParameters(params).toArray(), [SexpOpcodes.FlushElement], ...CONTENT.list(body), [SexpOpcodes.CloseElement]]);
  }
  Component({
    tag,
    params,
